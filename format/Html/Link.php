 ...
 * @standard PSR-2
 */
class Link extends AbstractFormatter implements FormatterInterface
{
  /**
  ...
  ): ?string
  {
    $template = incept('handlebars')->compile(
      file_get_contents(__DIR__ . '/template/link.html')
    );
  ...
  public static function getConfigFieldset(): array
  {
    return [
      FieldRegistry::makeField('text')
        ...
        ->setAttributes([ 'placeholder' => 'ie. /node/{{node_id}}' ]),
      FieldRegistry::makeField('text')
