...
$this('event')->on('sql-build-database', function (
  ...
  if (!isset($config[$key]['name'])) {
    ...
    $terminal->error('Database name not found.', false);
...
$this('event')->on('sql-build-schema', function (
  ...
  //if the schema created has an error
  if (!$response->isValid()) {
    $terminal->error('Some schemas were not installed', false);
    $errors = $response->getValidation();
    if (is_array($errors)) {
      foreach ($errors as $name => $message) {
        $terminal->error(sprintf(' - %s - %s', $name, $message), false);
      }
    }
